...
std::string IotHubClient::base64EncodeForSas(const std::string& input) {
    return base64Encode(reinterpret_cast<const unsigned char*>(input.c_str()), input.size());
}
...
std::string IotHubClient::generateSASToken(const std::string& resourceUri,
    ...
    std::string signature = base64EncodeForSas(std::string(reinterpret_cast<char*>(hmac), hmacLength));
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            ...
            if (response_code == 200 || response_code == 204) {
                ...
                std::cerr << "Error: Received HTTP " << response_code << " from IoT Hub." << std::endl;
