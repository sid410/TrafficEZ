...
public:
    ...
    convertStringVectorToInts(const std::vector<std::string>& input);
    static std::unordered_map<int, std::string>
    parseNames(const std::string& input);
    ...
    static void letterbox(const cv::Mat& image,
                          ...
                          bool scaleUp = true,
                          int stride = 32);
    static void scaleImage(cv::Mat& scaled_mask,
    ...
    // ops
    cv::Rect_<float> static scale_boxes(
    ...
    static void clip_boxes(std::vector<cv::Rect_<float>>& boxes,
                           const cv::Size& shape);
};
