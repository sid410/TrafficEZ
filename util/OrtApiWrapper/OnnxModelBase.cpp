    ...
    : modelPath_(modelPath)
{
    ...
    if(provider == OnnxProviders::CUDA.c_str())
    {
        ...
        {
            std::cout << "CUDA is not supported by your ONNXRuntime build. "
                         "Fallback to CPU."
                      << std::endl;
        }
        else
    ...
    {
        throw std::runtime_error("NotImplemented provider=" +
                                 std::string(provider));
    }
    ...
    std::vector<Ort::AllocatedStringPtr>
        inputNodeNameAllocatedStrings; // <-- newly added
    ...
    std::vector<Ort::AllocatedStringPtr>
        outputNodeNameAllocatedStrings; // <-- newly added
    Ort::AllocatorWithDefaultOptions output_names_allocator;
    for(int i = 0; i < outputNodesNum; i++)
    {
        auto output_name =
            session.GetOutputNameAllocated(i, output_names_allocator);
    ...
    for(const std::string& key : metadata_keys)
    {
        Ort::AllocatedStringPtr metadata_value =
            model_metadata.LookupCustomMetadataMapAllocated(key.c_str(),
                                                            metadata_allocator);
...
OnnxModelBase::forward(std::vector<Ort::Value>& inputTensors)
{
