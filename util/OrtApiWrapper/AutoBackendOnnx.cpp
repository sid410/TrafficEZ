    ...
    : OnnxModelBase(modelPath, logid, provider)
{
    ...
    if(imgsz_iterator != base_metadata.end())
    {
        ...
        std::vector<int> imgsz = YoloUtils::convertStringVectorToInts(
            YoloUtils::parseVectorString(imgsz_iterator->second));
        // set it here:
        if(imgsz_.empty())
        {
            imgsz_ = imgsz;
    ...
    {
        std::cerr << "Warning: Cannot get imgsz value from metadata"
                  << std::endl;
    }
    ...
    {
        std::cerr << "Warning: Cannot get stride value from metadata"
                  << std::endl;
    }
    ...
    if(names_item != base_metadata.end())
    {
        ...
        std::unordered_map<int, std::string> names =
            YoloUtils::parseNames(names_item->second);
    ...
    {
        std::cerr << "Warning: Cannot get names value from metadata"
                  << std::endl;
    }
    ...
    {
        std::cerr << "Warning: Cannot get nc value from metadata (probably "
                     "names wasn't set)"
                  << std::endl;
    }
    ...
    {
        std::cerr << "Warning: Cannot get task value from metadata"
                  << std::endl;
    }
...
AutoBackendOnnx::predict_once(const std::string& imagePath,
                              float& conf,
                              float& iou,
                              ...
                              int conversionCode)
{
    ...
    return predict_once(
        imageFilePath, conf, iou, mask_threshold, conversionCode);
}

std::vector<YoloResults>
AutoBackendOnnx::predict_once(const fs::path& imagePath,
                              float& conf,
                              float& iou,
                              float& mask_threshold,
                              int conversionCode)
{
    ...
    if(required_image_channels != image.channels())
    {
        const std::string& errorMessage =
            "Error: Number of image channels does not match the required "
            "channels.\n"
                                                       ...
                                                       int conversionCode)
{
    ...
    if(task_ == YoloTasks::SEGMENT)
    {
        ...
        std::vector<int> mask_sz = {
            1, (int) mask_shape[1], (int) mask_shape[2], (int) mask_shape[3]};
        cv::Mat output1 = cv::Mat(
            mask_sz, CV_32F, outputTensors[1].GetTensorMutableData<float>());
                                        ...
                                        float mask_threshold /* = 0.5f */)
{
    ...
    for(int r = 0; r < rows; ++r)
    {
        ...
        {
            masks.push_back(std::vector<float>(pdata + 4 + class_names_num,
                                               pdata + data_width));
            ...
            cv::Rect_<float> scaled_bbox =
                YoloUtils::scale_boxes(getCvSize(), bbox, image_info.raw_size);
    ...
    cv::Mat proto = temp_mask.reshape(
        0,
        {masks_features_num, downsampled_size.width * downsampled_size.height});

    for(int i = 0; i < nms_result.size(); ++i)
    {
        int idx = nms_result[i];
        boxes[idx] = boxes[idx] & cv::Rect(0,
                                           0,
                                           image_info.raw_size.width,
...

{
    ...
    cv::Rect_<float> downsampled_bbox =
        YoloUtils::scale_boxes(img0_shape, bound_float, downsampled_size);
    cv::Size bound_size = cv::Size(mw, mh);
    YoloUtils::clip_boxes(downsampled_bbox, bound_size);

    cv::Mat matmul_res = (masks_features * proto).t();
    matmul_res = matmul_res.reshape(
        1, {downsampled_size.height, downsampled_size.width});
    ...
    cv::Rect_<float> input_bbox =
        YoloUtils::scale_boxes(img0_shape, bound_float, img1_shape);
    cv::resize(
        sigmoid_mask, resized_mask, img1_shape, 0, 0, cv::INTER_LANCZOS4);
